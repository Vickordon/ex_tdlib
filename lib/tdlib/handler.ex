defmodule TDLib.Handler do
  ...
  defp match(:object, json, prefix) do
     type_string = Map.get(json, "@type")
  # Приводимо до PascalCase (назва модуля)
  pascal_case = Macro.camelize(type_string)

  # Розбиваємо pascal_case на перший символ і решту (для збирання модуля з префіксом)
  <<char::binary-size(1), rest::binary>> = pascal_case
    string = prefix <> char <> rest
    module = String.to_atom(string)
    # IO.inspect(module, label: "Модуль")
