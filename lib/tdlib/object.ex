...
defmodule ChatPosition do
  @moduledoc "Позиція чату в списку"

  defstruct [
    "@type": "chatPosition",
    list: nil,          # Тип списку чату (наприклад, chatListMain)
    order: "",          # Порядок чату у списку
    is_pinned: false    # Чи закріплений чат угорі
  ]
end


  defmodule UpdateChatPosition do
  @moduledoc "Оновлення позиції чату в списку чатів"

  defstruct [
    "@type": "updateChatPosition",
    chat_id: 0,        # Унікальний ідентифікатор чату
    position: nil      # Позиція чату у списку (структура chatPosition)
  ]
end


defmodule UpdateUnreadMessageCount do
  @moduledoc "Оновлення кількості непрочитаних повідомлень у списку чатів"

  defstruct [
    "@type": "updateUnreadMessageCount",
    chat_list: nil,           # Список чатів, до якого належить лічильник (наприклад, %{:"@type" => "chatListMain"})
    unread_count: 0,          # Загальна кількість непрочитаних повідомлень
    unread_unmuted_count: 0   # Кількість непрочитаних повідомлень у немовчазних чатах
  ]
end


  defmodule MessagePoll do
  @moduledoc "Контент повідомлення: опитування (poll)"

  defstruct [
    "@type": "messagePoll",
    poll: nil  # Структура TDLib.Object.Poll
  ]
end


  defmodule FormattedText do
  @moduledoc "Форматований текст, який може містити стилі чи посилання"

  defstruct [
    "@type": "formattedText",
    text: "",                         # Текст повідомлення
    entities: []                      # Список об'єктів стилізації (наприклад, bold, url тощо)
  ]
end


  defmodule PollTypeQuiz do
  @moduledoc "Тип опитування: вікторина (Quiz)"

  defstruct [
    "@type": "pollTypeQuiz",
    correct_option_id: -1,            # Індекс правильної відповіді, або -1
    explanation: nil                  # Пояснення (FormattedText)
  ]
end
...
defmodule PollTypeQuiz do
  ...
  @moduledoc "Один з варіантів відповіді в опитуванні"

  defstruct [
    "@type": "pollOption",
    text: nil,                         # Форматований текст (FormattedText)
    voter_count: 0,                    # Кількість голосів
    vote_percentage: 0,                # Відсоток голосів (0–100)
    is_chosen: false,                  # Чи вибрав користувач цей варіант
    is_being_chosen: false             # Чи змінює користувач голос зараз
  ]
end


defmodule Poll do
  @moduledoc "Опитування, вкладене в повідомлення"

  defstruct [
    "@type": "poll",
    id: "",                            # Унікальний ідентифікатор опитування (рядок)
    question: nil,                     # Запитання (FormattedText)
    options: [],                       # Список варіантів (PollOption)
    total_voter_count: 0,              # Загальна кількість голосуючих
    recent_voter_ids: [],             # Ідентифікатори нещодавніх голосуючих
    is_anonymous: true,                # Чи є опитування анонімним
    type: nil,                         # Тип опитування (PollTypeRegular або PollTypeQuiz)
    open_period: 0,                    # Тривалість активності після публікації (секунди)
    close_date: 0,                     # Дата закриття (UNIX-час)
    is_closed: false                   # Чи є опитування закритим
  ]
end


defmodule UpdateFileDownloads do
  @moduledoc "Оновлення інформації про завантаження файлів"

  defstruct [
    "@type": "updateFileDownloads",
    total_size: 0,         # Загальний розмір файлів, які потрібно завантажити, у байтах
    total_count: 0,        # Загальна кількість файлів
    downloaded_size: 0     # Загальний розмір вже завантажених байтів
  ]
end
...
defmodule UpdateDefaultBackground do
  @moduledoc "Оновлення фонового зображення за замовчуванням"

  defstruct [
    "@type": "updateDefaultBackground",
    for_dark_theme: false  # true, якщо фонове зображення для темної теми
  ]
end


  defmodule UpdateAttachmentMenuBots do
  @moduledoc "Оновлення списку ботів для меню вкладень"

  defstruct [
    "@type": "updateAttachmentMenuBots",
    bots: []  # Список ботів (типово порожній), кожен — %TDLib.Object.AttachmentMenuBot{}
  ]
end


defmodule UpdateSpeechRecognitionTrial do
  @moduledoc "Оновлення інформації про пробний період розпізнавання мовлення"

  defstruct [
    "@type": "updateSpeechRecognitionTrial",
    max_media_duration: 0,   # Максимальна тривалість медіа для розпізнавання (секунди)
    weekly_count: 0,         # Кількість тижневих спроб
    left_count: 0,           # Залишок доступних спроб
    next_reset_date: 0       # Наступна дата скидання лічильника (Unix timestamp)
  ]
end

  defmodule UpdateProfileAccentColors do
  @moduledoc "Оновлення акцентних кольорів профілю"

  defstruct [
    "@type": "updateProfileAccentColors",
    colors: [] # список %TDLib.Object.ProfileAccentColor{}
  ]
end

defmodule ProfileAccentColor do
  @moduledoc "Акцентний колір профілю"

  defstruct [
    "@type": "profileAccentColor",
    id: 0,
    light_theme_colors: nil,  # %TDLib.Object.ProfileAccentColors{}
    dark_theme_colors: nil,   # %TDLib.Object.ProfileAccentColors{}
...
defmodule ProfileAccentColors do
  @moduledoc "Кольори акценту профілю"

  defstruct [
    "@type": "profileAccentColors",
    palette_colors: [],    # список integer (кольори)
    background_colors: [], # список integer (кольори)
    story_colors: []       # список integer (кольори)
  ]
end
  ...
  @moduledoc "Оновлення: чат додано до списку"

  defstruct [
    "@type": "updateChatAddedToList",
    chat_id: 0,
    chat_list: nil  # %TDLib.Object.ChatListMain{} або інший тип списку
  ]
end

defmodule ChatListMain do
  @moduledoc "Основний список чатів"
...
defmodule UpdateChatNotificationSettings do
  @moduledoc "Оновлення налаштувань повідомлень чату"

  defstruct [
    "@type": "updateChatNotificationSettings",
    chat_id: 0,
    notification_settings: nil  # %TDLib.Object.ChatNotificationSettings{}
  ]
end

defmodule ChatNotificationSettings do
  @moduledoc "Налаштування повідомлень чату"
...
defmodule ChatAvailableReactionsAll do
  @moduledoc "Всі доступні реакції чату"
...
defmodule AccentColor do
  @moduledoc "Акцентний колір з налаштуваннями для світлої та темної тем"

  defstruct [
    "@type": "accentColor",
    id: 0,
    built_in_accent_color_id: 0,
    light_theme_colors: [],    # список integer (кольори для світлої теми)
    dark_theme_colors: [],     # список integer (кольори для темної теми)
    min_channel_chat_boost_level: 0
  ]
end
  ...
  @moduledoc "Оновлення акцентних кольорів у Telegram"

  defstruct [
    "@type": "updateAccentColors",
    colors: [],                  # список %TDLib.Object.AccentColor{}
    available_accent_color_ids: [] # список integer
  ]
end
  ...
  @moduledoc "Мініатюра медіафайлу (наприклад, зображення або відео)"

  defstruct [
    "@type": "thumbnail",
    format: nil,   # %TDLib.Object.ThumbnailFormat* (наприклад, ThumbnailFormatJpeg)
    width: 0,
    height: 0,
    file: nil      # %TDLib.Object.File{}
  ]
end

defmodule ThumbnailFormatJpeg do
  @moduledoc "Формат мініатюри у вигляді JPEG"
  ...
  @moduledoc "Наявність відкладених чи непрочитаних сповіщень"
...
defmodule MessageInteractionInfo do
  @moduledoc "Інформація про взаємодії з повідомленням"

  defstruct [
    "@type": "messageInteractionInfo",
    view_count: 0,
    forward_count: 0,
    reactions: nil,               # %TDLib.Object.MessageReactions{}
    recent_reactions: [],         # список %TDLib.Object.MessageReaction{}
    last_read_reaction: nil,      # %TDLib.Object.MessageReaction{}
    can_get_added_reactions: false
  ]
end

defmodule MessageReactions do
  @moduledoc "Список реакцій до повідомлення"

  defstruct [
    "@type": "messageReactions",
    reactions: [],                # список %TDLib.Object.MessageReaction{}
    are_tags: false,
    paid_reactors: [],            # список ID (можливо user_ids або objects)
    can_get_added_reactions: false
  ]
end

defmodule MessageReaction do
  @moduledoc "Окрема реакція до повідомлення"

  defstruct [
    "@type": "messageReaction",
    type: nil,                    # %TDLib.Object.ReactionTypeEmoji{}
    total_count: 0,
    is_chosen: false,
    recent_sender_ids: []         # список %TDLib.Object.MessageSenderUser{}
  ]
end


defmodule MessageSenderUser do
  @moduledoc "Відправник — користувач"
  ...
  @moduledoc """
  Represents a reaction type with an emoji.

  [TDLib документація](https://core.telegram.org/tdlib/docs/classtd_1_1td__api_1_1reaction_type_emoji.html)

  | Name  | Type   | Description               |
  |-------|--------|---------------------------|
  | emoji | String | Emoji, наприклад "❤️"     |
  """

  defstruct "@type": "reactionTypeEmoji",
            emoji: ""
end
  ...
  @moduledoc """
  Частковий набір доступних реакцій у чаті.

  | Назва            | Тип    | Опис                                 |
  |------------------|--------|-------------------------------------|
  | reactions        | list   | Список реакцій (emoji)               |
  | max_reaction_count | integer| Максимальна кількість реакцій на повідомлення |

  Документація TDLib:
  https://core.telegram.org/tdlib/docs/classtd_1_1td__api_1_1chat_available_reactions_some.html
  """
  ...
  @moduledoc """
  Оновлення списку доступних реакцій у чаті.

  | Назва             | Тип    | Опис                                   |
  |-------------------|--------|---------------------------------------|
  | chat_id           | integer| Ідентифікатор чату                    |
  | available_reactions| map    | Інформація про доступні реакції       |

  Документація TDLib:
  https://core.telegram.org/tdlib/docs/classtd_1_1td__api_1_1update_chat_available_reactions.html
  """
  ...
  @moduledoc """
  Оновлення, яке інформує про тип реакції за замовчуванням.

  | Name          | Type   | Description                         |
  |---------------|--------|-----------------------------------|
  | reaction_type | map    | Тип реакції (наприклад, emoji)    |

  Документація TDLib:
  https://core.telegram.org/tdlib/docs/classtd_1_1td__api_1_1update_default_reaction_type.html
  """

  defstruct "@type": "updateDefaultReactionType",
            reaction_type: nil
end
...
defmodule ChatPhotoInfo do
  @moduledoc """
  Represents information about a chat photo.

  Документація:
  https://core.telegram.org/tdlib/docs/classtd_1_1td__api_1_1chat_photo_info.html

  | Назва         | Тип    | Опис                                                                 |
  |---------------|--------|----------------------------------------------------------------------|
  | small         | map    | Маленьке фото чату (тип `File`)                                      |
  | big           | map    | Велике фото чату (тип `File`)                                        |
  | minithumbnail | map    | Мініатюра (тип `Minithumbnail`) або `nil`                            |
  | has_animation | bool   | Чи має анімацію                                                       |
  | is_personal   | bool   | Чи є персональним для користувача                                    |
  """
...
defmodule UpdateDefaultReactionType do
  @moduledoc """
  Represents an update about the default reaction type.

  | Name          | Type | Description                        |
  |---------------|------|----------------------------------|
  | reaction_type | map  | The new default reaction type    |

  Детальніше у документації Telegram TDLib:
  https://core.telegram.org/tdlib/docs/classtd_1_1td__api_1_1update_default_reaction_type.html
  """
...
defmodule UpdateMessageInteractionInfo do
  @moduledoc """
  Represents an update about message interaction information.

  | Name            | Type | Description                                            |
  |-----------------|------|--------------------------------------------------------|
  | chat_id         | int  | Identifier of the chat                                 |
  | message_id      | int  | Identifier of the message                              |
  | interaction_info| map  | Information about interactions with the message (views, forwards, reactions) |

  Більш детально можна подивитись у документації Telegram TDLib:
  https://core.telegram.org/tdlib/docs/classtd_1_1td__api_1_1update_message_interaction_info.html
  """
...
defmodule UpdateSelectedBackground do
  @moduledoc """
  Represents an update about the currently selected background.

  | Name       | Type   | Description                         |
  |------------|--------|-----------------------------------|
  | background | map    | The selected background information |

  Більш детально можна подивитись у документації Telegram TDLib:
  https://core.telegram.org/tdlib/docs/classtd_1_1td__api_1_1update_selected_background.html
  """

  defstruct "@type": "updateSelectedBackground",
            background: nil
end

defmodule UpdateAnimationSearchParameters do
  @moduledoc """
  Provides updated animation search parameters.

  | Name     | Type   | Description                   |
  |----------|--------|-------------------------------|
  | provider | string | Provider of animation search  |
  | emojis   | list   | List of emojis for searching  |
  ...
  defstruct "@type": "updateAnimationSearchParameters",
            provider: nil,
            emojis: nil
end
